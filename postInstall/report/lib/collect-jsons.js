    ...
    if (files.length > 0) {
        files.map(file => {
            ...
            JSON.parse(data).map(json => {
                ...
                // Set test time in to json, will add it later, start
                // const fileName = file.substring(file.lastIndexOf('/s') + 2, file.indexOf('.json'));
                // const testTime = moment(fileName * 1).format('YYYY-MM-DD HH:mm:ss Z');
                // json = Object.assign({ testTime: testTime }, json);
                // end

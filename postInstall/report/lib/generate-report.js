...
let GROUPS_SCENARIOS_OVERVIEW_CHART_TEMPLATE = 'components/groups-scenarios-overview.chart.tmpl';
...
=====================================================================================\n`));
	...
	function _parseFeatures(suite) {
      ...
      originalScenarios.failed = suite.scenarios.failed;
      originalScenarios.ambiguous = suite.scenarios.ambiguous;
      originalScenarios.notDefined = suite.scenarios.notDefined;
      originalScenarios.pending = suite.scenarios.pending;
      originalScenarios.skipped = suite.scenarios.skipped;
      originalScenarios.passed = suite.scenarios.passed;
      originalScenarios.total = suite.scenarios.total;
			...
			// Groups information
      if(feature.name.indexOf('[') > -1 && feature.name.indexOf(']') > -1) {
        const tmpGroupName = feature.name.substring(feature.name.indexOf('[')+1, feature.name.indexOf(']')).replace(/ /g, '').replace(/-/g, '');
        if(suite.groups.hasOwnProperty(tmpGroupName)) {
          suite.groups[tmpGroupName].failed += suite.scenarios.failed - originalScenarios.failed;
          suite.groups[tmpGroupName].ambiguous += suite.scenarios.ambiguous - originalScenarios.ambiguous;
          suite.groups[tmpGroupName].notDefined += suite.scenarios.notDefined - originalScenarios.notDefined;
          suite.groups[tmpGroupName].pending += suite.scenarios.pending - originalScenarios.pending;
          suite.groups[tmpGroupName].skipped += suite.scenarios.skipped - originalScenarios.skipped;
          suite.groups[tmpGroupName].passed += suite.scenarios.passed - originalScenarios.passed;
          suite.groups[tmpGroupName].total += suite.scenarios.total - originalScenarios.total;
        } else {
          suite.groups[tmpGroupName] = {
            failed: suite.scenarios.failed - originalScenarios.failed,
            ambiguous: suite.scenarios.ambiguous - originalScenarios.ambiguous,
            notDefined: suite.scenarios.notDefined - originalScenarios.notDefined,
            pending: suite.scenarios.pending - originalScenarios.pending,
            skipped: suite.scenarios.skipped - originalScenarios.skipped,
            passed: suite.scenarios.passed - originalScenarios.passed,
            total: suite.scenarios.total - originalScenarios.total
          };
        }

        suite.groups[tmpGroupName].ambiguousPercentage = _calculatePercentage(suite.groups[tmpGroupName].ambiguous, suite.groups[tmpGroupName].total);
        suite.groups[tmpGroupName].failedPercentage = _calculatePercentage(suite.groups[tmpGroupName].failed, suite.groups[tmpGroupName].total);
        suite.groups[tmpGroupName].notdefinedPercentage = _calculatePercentage(suite.groups[tmpGroupName].notDefined, suite.groups[tmpGroupName].total);
        suite.groups[tmpGroupName].passedPercentage = _calculatePercentage(suite.groups[tmpGroupName].passed, suite.groups[tmpGroupName].total);
        suite.groups[tmpGroupName].pendingPercentage = _calculatePercentage(suite.groups[tmpGroupName].pending, suite.groups[tmpGroupName].total);
        suite.groups[tmpGroupName].skippedPercentage = _calculatePercentage(suite.groups[tmpGroupName].skipped, suite.groups[tmpGroupName].total);
      } else {
        if (!suite.groups.Others) {
          suite.groups.Others = {
            failed: 0,
            ambiguous: 0,
            notDefined: 0,
        ...
        suite.groups.Others.failed += suite.scenarios.failed - originalScenarios.failed;
        suite.groups.Others.ambiguous += suite.scenarios.ambiguous - originalScenarios.ambiguous;
        suite.groups.Others.notDefined += suite.scenarios.notDefined - originalScenarios.notDefined;
        suite.groups.Others.pending += suite.scenarios.pending - originalScenarios.pending;
        suite.groups.Others.skipped += suite.scenarios.skipped - originalScenarios.skipped;
        suite.groups.Others.passed += suite.scenarios.passed - originalScenarios.passed;
        suite.groups.Others.total += suite.scenarios.total - originalScenarios.total;

        suite.groups.Others.ambiguousPercentage = _calculatePercentage(suite.groups.Others.ambiguous, suite.groups.Others.total);
        suite.groups.Others.failedPercentage = _calculatePercentage(suite.groups.Others.failed, suite.groups.Others.total);
        suite.groups.Others.notdefinedPercentage = _calculatePercentage(suite.groups.Others.notDefined, suite.groups.Others.total);
        suite.groups.Others.passedPercentage = _calculatePercentage(suite.groups.Others.passed, suite.groups.Others.total);
        suite.groups.Others.pendingPercentage = _calculatePercentage(suite.groups.Others.pending, suite.groups.Others.total);
        suite.groups.Others.skippedPercentage = _calculatePercentage(suite.groups.Others.skipped, suite.groups.Others.total);
      }
		});
	}
    ...
    const originalObj = {
      ...
      groupLength: Object.getOwnPropertyNames(suite.groups).length,
      _: _
    };

		// let groupsStr = '';
    _.each(suite.groups, (groupValue, groupKey) => {
      originalObj[`${groupKey}`] = _.template(_readTemplateFile(GROUPS_SCENARIOS_OVERVIEW_CHART_TEMPLATE))({
        overviewPage: true,
        groupName: groupKey,
        scenarios: groupValue,
        _: _
      });
    });

    const resultObj = originalObj;

		fs.writeFileSync(
			featuresOverviewIndex,
			_.template(_readTemplateFile(FEATURES_OVERVIEW_INDEX_TEMPLATE))(resultObj)
		);
	}
